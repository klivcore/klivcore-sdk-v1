...
// ../../node_modules/.bun/react@19.2.7/node_modules/react/cjs/react.production.js
...
// ../../node_modules/.bun/react@19.2.7/node_modules/react/cjs/react-jsx-runtime.production.js
...
// ../../node_modules/.bun/react-dom@19.2.7+e14d3f224186685e/node_modules/react-dom/cjs/react-dom.production.js
...
// ../../node_modules/.bun/react-dom@19.2.7+e14d3f224186685e/node_modules/react-dom/cjs/react-dom-client.production.js
...
// ../../node_modules/.bun/react-dom@19.2.7+e14d3f224186685e/node_modules/react-dom/client.js
...
function parseLiveComponentFrameInit(value) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw invalidInit();
  const candidate = value;
  if (Object.keys(candidate).sort().join("\x00") !== [
    "componentCss",
    "componentJavaScript",
    ...
    "typeId",
    "version"
  ].sort().join("\x00") || candidate.type !== "init" || candidate.version !== 1 || typeof candidate.typeId !== "string" || !TYPE_ID.test(candidate.typeId) || typeof candidate.implementationRevision !== "string" || !SHA256.test(candidate.implementationRevision) || !(candidate.componentCss instanceof ArrayBuffer) || candidate.componentCss.byteLength > MAX_COMPONENT_CSS_BYTES || !(candidate.componentJavaScript instanceof ArrayBuffer) || candidate.componentJavaScript.byteLength < 1 || candidate.componentJavaScript.byteLength > MAX_COMPONENT_JAVASCRIPT_BYTES || !isBoundedJsonObject(candidate.props))
    throw invalidInit();
  return Object.freeze({
    componentCss: candidate.componentCss,
    componentJavaScript: candidate.componentJavaScript,
...
function initializeLiveComponentFrame(port2, frameRoot) {
  if (!(port2 instanceof MessagePort) || !(frameRoot instanceof Element))
    throw new TypeError("Live component frame bootstrap is invalid");
  let initialized = false;
  port2.onmessage = (event) => {
    if (initialized)
      return;
    initialized = true;
    mount(port2, frameRoot, event.data);
  };
  port2.start();
}
...
async function mount(port2, frameRoot, rawInit) {
  let componentUrl;
  let settled = false;
  const reject = (error) => {
    if (settled)
      return;
    settled = true;
    port2.postMessage(Object.freeze({
  ...
  try {
    ...
    const cssText = new TextDecoder("utf-8", { fatal: true }).decode(init.componentCss);
    const ready = () => {
      if (settled)
        return;
      settled = true;
      port2.postMessage(Object.freeze({
        implementationRevision: init.implementationRevision,
        type: "ready",
        typeId: init.typeId,
        version: 1
      }));
    };
    import_client.createRoot(frameRoot, { onCaughtError: reject, onUncaughtError: reject }).render(React3.createElement(FrameErrorBoundary, { onError: reject }, cssText ? React3.createElement("style", null, cssText) : null, definition.render(React3, init.props), React3.createElement(PostCommitSentinel, { onCommit: ready })));
  } catch (error) {
    reject(error);
  } finally {
    if (componentUrl)
...
function PostCommitSentinel({ onCommit }) {
  React3.useLayoutEffect(onCommit, [onCommit]);
  return null;
}
function parseComponentModule(value, init) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("Component module is invalid");
  const module = value;
  if (Object.keys(module).join("\x00") !== "components")
    throw new Error("Component module is invalid");
  ...
  if (Object.keys(candidate).sort().join("\x00") !== ["implementationRevision", "render", "renderMode", "typeId"].sort().join("\x00") || candidate.typeId !== init.typeId || candidate.implementationRevision !== init.implementationRevision || candidate.renderMode !== "element" || typeof candidate.render !== "function") {
    throw new Error("Component module is invalid");
  }
  return candidate;
}
function isBoundedJsonObject(value) {
  if (!isJsonValue(value, new Set, 0) || !value || Array.isArray(value))
    return false;
  try {
    const json = JSON.stringify(value);
    return typeof json === "string" && encoder.encode(json).byteLength <= MAX_PROPS_JSON_BYTES;
  } catch {
    return false;
  }
}
...
function boundedError(error) {
  const message = error instanceof Error ? error.message : "Live component runtime failed";
  return message.replace(/[\r\n\t]+/gu, " ").slice(0, MAX_ERROR_MESSAGE_LENGTH) || "Live component runtime failed";
}
function invalidInit() {
  return new TypeError("Live component frame init is invalid");
}
export {
