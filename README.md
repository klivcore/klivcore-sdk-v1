...
For managed Quick Tunnels, `start-realm` is an idempotent coordinator over two deterministic named tmux sessions: one owns `cloudflared`, and one owns the Realm. Closing the invoking terminal or disconnecting SSH does not stop either session. A rerun validates and reuses each healthy session, including the existing Quick Tunnel origin, rather than allocating duplicate resources. After local health succeeds, the Realm worker reports a newly allocated hostname as publicly propagating and keeps both components running while fresh resolver processes retry only public health. Use the printed component-specific tmux commands to inspect or restart only a broken Realm; stopping the tunnel intentionally also requires restarting the origin-bound Realm. These sessions do not survive an EC2 reboot. Use a named tunnel and boot-managed service for reboot durability and stable DNS.
