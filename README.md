...
For managed Quick Tunnels, `start-realm` is an idempotent coordinator over deterministic named tmux sessions. The Realm tunnel and Realm worker are independent. When `desktop.ssh` is configured, an SSH-only Core relay and its own Quick Tunnel run in two additional sessions. Closing the invoking terminal or disconnecting SSH does not stop them. A rerun validates and reuses every healthy session rather than allocating duplicate resources. A changed Realm runtime revision replaces only the Realm worker: the SSH Core relay, SSH Quick Tunnel, existing WSS connections, and active SSH sessions remain running. Use the printed component-specific tmux commands to inspect or restart only a broken component. These sessions do not survive an EC2 reboot. Use named tunnels and boot-managed services for reboot durability and stable DNS.
...
A route component exports `mount(host)`. `host.root` is its isolated ShadowRoot and `host.services` contains only its declared named services. Each service exposes its non-authoritative same-Realm `/:port` endpoint for identity plus host-mediated `request(path, init)` and `openChannel(path, handlers)` operations. App V2 keeps the route-scoped relay grant inside those functions; bare endpoint requests and grants issued to another route, session, or port fail closed. Components must consume those operations instead of calling `fetch` or `WebSocket` with the endpoint. Existing routes without services receive an empty map.
...
Configure `desktop.ssh` with the fixed private SSH target reachable by the Realm process. For an EC2 instance hosting its own Realm, use `127.0.0.1`, port `22`, the ordinary SSH user, and the absolute Realm working directory. On Linux loopback targets, `start-realm` uses passwordless `sudo` to install, validate, reload, and verify the `sshd` integration that preserves the operator key file and adds the Realm-owned `<stateDir>/desktop-authorized-keys` projection. Keep SSH loopback-only; do not put passwords, private keys, public tunnel URLs, or shell commands in the Realm config.

After passkey sign-in, choose **Connect Desktop** in the Realm menu. It copies a command containing a five-minute one-use pairing capability. Run that command on the machine containing Hermes Desktop. The command atomically generates a dedicated mode-`0600` Ed25519 key locally when needed, sends only its public key in the authenticated one-use pairing transaction, creates a mode-`0600` relay profile, and installs a managed `~/.ssh/config` host named `klivcore-<realm-id>` that points to `127.0.0.1:2222`. It then remains in the foreground as that loopback listener, prints connection lifecycle messages, and carries every accepted SSH connection over a separately authenticated WSS connection through the independent SSH Quick Tunnel. Keep the terminal open while using Desktop and press Ctrl+C to stop local access without stopping any remote component. To restart the listener without pairing again, run the same immutable SDK package with `connect-desktop listen <realm-id>`. Existing profiles paired before SSH isolation must be paired once more to store the dedicated relay endpoint; the managed SSH key is reused.
