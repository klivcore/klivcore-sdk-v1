...
const PASSKEY_BROWSER_JS = String.raw`(() => {
  ...
  async function post(path, value) { const response = await fetch(path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(value) }); const payload = response.status === 204 ? undefined : await response.json().catch(() => undefined); if (!response.ok) { const reason = payload && typeof payload.reason === 'string' ? ': ' + payload.reason : ''; throw new Error('Request failed (' + response.status + ')' + reason); } return payload; }
